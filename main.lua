...
function love.update(dt)
    ...
    if gameMode == 'Singleplayer' then
        ...
        local speedFactor = math.max(0.8, math.min(1, math.abs(distance) / 10)) -- Dynamic speed factor
        local moveSpeed = PADDLE_SPEED * speedFactor                            -- Apply dynamic factor

        if math.abs(distance) > 12 then                                         -- Threshold to prevent tiny, unnecessary movements
